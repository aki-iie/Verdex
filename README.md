...
실제 사진이 없는 글에 쓰는 브랜드 기본 이미지입니다. **회사 로고(잎 + 워드마크)를
그대로** 가운데 두고, 모서리에 서브바와 같은 잎맥 한 다발을 넣었습니다.

로고 사용은 `images/brand/BRAND.md` 규칙을 따릅니다 — 남색 배경에는 `verdex-logo-ondark.svg`
(잎은 초록, 글자만 흰색), 밝은 배경에는 `verdex-logo-color.svg`. 같은 문서의
"최소 가로 96px" 규칙을 지키려고 로고를 가로 64%로 크게 배치했습니다
(목록 썸네일 가로 약 180px 기준 로고 약 115px).
